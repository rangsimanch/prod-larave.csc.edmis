                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.file-managers.update", [$fileManager->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('file_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="file_name" id="file_name" value="{{ old('file_name', $fileManager->file_name) }}">
                        ...
                        <div class="form-group {{ $errors->has('code') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="code" id="code" value="{{ old('code', $fileManager->code) }}">
                            @if($errors->has('code'))
                        ...
                        <div class="form-group {{ $errors->has('construction_contracts') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="construction_contracts[]" id="construction_contracts" multiple>
                                @foreach($construction_contracts as $id => $construction_contract)
                                    <option value="{{ $id }}" {{ (in_array($id, old('construction_contracts', [])) || $fileManager->construction_contracts->contains($id)) ? 'selected' : '' }}>{{ $construction_contract }}</option>
                                @endforeach
                            </select>
