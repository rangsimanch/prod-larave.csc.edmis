        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.meetingOther.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.meeting-others.update", [$meetingOther->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('document_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="document_name" id="document_name" value="{{ old('document_name', $meetingOther->document_name) }}">
                        ...
                        <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="date" id="date" value="{{ old('date', $meetingOther->date) }}">
                            @if($errors->has('date'))
                        ...
                        <div class="form-group {{ $errors->has('note') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="note" id="note">{{ old('note', $meetingOther->note) }}</textarea>
                            @if($errors->has('note'))
                        ...
                        <div class="form-group {{ $errors->has('construction_contract') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="construction_contract_id" id="construction_contract_id">
                                @foreach($construction_contracts as $id => $construction_contract)
                                    <option value="{{ $id }}" {{ ($meetingOther->construction_contract ? $meetingOther->construction_contract->id : old('construction_contract_id')) == $id ? 'selected' : '' }}>{{ $construction_contract }}</option>
                                @endforeach
                            </select>
