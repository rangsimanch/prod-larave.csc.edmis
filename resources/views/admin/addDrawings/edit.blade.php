        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.addDrawing.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.add-drawings.update", [$addDrawing->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('drawing_title') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="drawing_title" id="drawing_title" value="{{ old('drawing_title', $addDrawing->drawing_title) }}">
                        ...
                        <div class="form-group {{ $errors->has('activity_type') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="activity_type_id" id="activity_type_id">
                                @foreach($activity_types as $id => $activity_type)
                                    <option value="{{ $id }}" {{ ($addDrawing->activity_type ? $addDrawing->activity_type->id : old('activity_type_id')) == $id ? 'selected' : '' }}>{{ $activity_type }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('work_type') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="work_type_id" id="work_type_id">
                                @foreach($work_types as $id => $work_type)
                                    <option value="{{ $id }}" {{ ($addDrawing->work_type ? $addDrawing->work_type->id : old('work_type_id')) == $id ? 'selected' : '' }}>{{ $work_type }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('coding_of_drawing') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="coding_of_drawing" id="coding_of_drawing" value="{{ old('coding_of_drawing', $addDrawing->coding_of_drawing) }}">
                        ...
                        <div class="form-group {{ $errors->has('location') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="location" id="location" value="{{ old('location', $addDrawing->location) }}">
