        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.jobtitles.update", [$jobtitle->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.jobtitle.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $jobtitle->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('departments') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="departments[]" id="departments" multiple required>
                                @foreach($departments as $id => $department)
                                    <option value="{{ $id }}" {{ (in_array($id, old('departments', [])) || $jobtitle->departments->contains($id)) ? 'selected' : '' }}>{{ $department }}</option>
                                @endforeach
                            </select>
