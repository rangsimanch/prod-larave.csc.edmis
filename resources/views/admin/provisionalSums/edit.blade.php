        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.provisionalSum.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.provisional-sums.update", [$provisionalSum->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('bill_no') ? 'has-error' : '' }}">
                            <label for="bill_no">{{ trans('cruds.provisionalSum.fields.bill_no') }}</label>
                            <input class="form-control" type="text" name="bill_no" id="bill_no" value="{{ old('bill_no', $provisionalSum->bill_no) }}">
                        ...
                        <div class="form-group {{ $errors->has('item_no') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="item_no" id="item_no" value="{{ old('item_no', $provisionalSum->item_no) }}">
                        ...
                        <div class="form-group {{ $errors->has('construction_contract') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="construction_contract_id" id="construction_contract_id">
                                @foreach($construction_contracts as $id => $construction_contract)
                                    <option value="{{ $id }}" {{ ($provisionalSum->construction_contract ? $provisionalSum->construction_contract->id : old('construction_contract_id')) == $id ? 'selected' : '' }}>{{ $construction_contract }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('name_of_ps') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="name_of_ps" id="name_of_ps" value="{{ old('name_of_ps', $provisionalSum->name_of_ps) }}">
