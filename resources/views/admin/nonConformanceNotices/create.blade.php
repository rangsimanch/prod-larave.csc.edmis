        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.nonConformanceNotice.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.non-conformance-notices.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('attachment_description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="attachment_description" id="attachment_description">{{ old('attachment_description') }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('status_ncn') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status_ncn" id="status_ncn">
                                ...
                                @foreach(App\NonConformanceNotice::STATUS_NCN_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status_ncn', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('csc_issuers') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="csc_issuers_id" id="csc_issuers_id">
                                @foreach($csc_issuers as $id => $csc_issuers)
                                    <option value="{{ $id }}" {{ old('csc_issuers_id') == $id ? 'selected' : '' }}>{{ $csc_issuers }}</option>
                                @endforeach
                            </select>
