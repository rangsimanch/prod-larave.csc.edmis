        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.contractAndComponent.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.contract-and-components.update", [$contractAndComponent->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('document_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="document_name" id="document_name" value="{{ old('document_name', $contractAndComponent->document_name) }}">
                        ...
                        <div class="form-group {{ $errors->has('document_code') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="document_code" id="document_code" value="{{ old('document_code', $contractAndComponent->document_code) }}">
                        ...
                        <div class="form-group {{ $errors->has('construction_contract') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="construction_contract_id" id="construction_contract_id">
                                @foreach($construction_contracts as $id => $construction_contract)
                                    <option value="{{ $id }}" {{ (old('construction_contract_id') ? old('construction_contract_id') : $contractAndComponent->construction_contract->id ?? '') == $id ? 'selected' : '' }}>{{ $construction_contract }}</option>
                                @endforeach
                            </select>
