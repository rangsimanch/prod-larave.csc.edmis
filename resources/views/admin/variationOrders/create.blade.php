        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.variationOrder.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.variation-orders.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('type_of_vo') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="type_of_vo" id="type_of_vo">
                                ...
                                @foreach(App\VariationOrder::TYPE_OF_VO_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('type_of_vo', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('operation_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="operation_date" id="operation_date" value="{{ old('operation_date') }}">
