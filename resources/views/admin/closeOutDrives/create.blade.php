        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.closeOutDrive.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.close-out-drives.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('filename') ? 'has-error' : '' }}">
                            <label class="required" for="filename">{{ trans('cruds.closeOutDrive.fields.filename') }}</label>
                            <input class="form-control" type="text" name="filename" id="filename" value="{{ old('filename', '') }}" required>
