...
@section('content')
<div class="content">
    @can('srt_pe_document_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.srt-pe-documents.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.srtPeDocument.title_singular') }}
                </a>
                <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                    {{ trans('global.app_csvImport') }}
                </button>
                @include('csvImport.modal', ['model' => 'SrtPeDocument', 'route' => 'admin.srt-pe-documents.parseCsvImport'])
            </div>
        </div>
    @endcan
    <div class="row">
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('cruds.srtPeDocument.title_singular') }} {{ trans('global.list') }}
                </div>
                <div class="panel-body">
                    <table class=" table nowrap table-bordered table-striped table-hover ajaxTable datatable datatable-SrtPeDocument">
                        <thead>
                            <tr>
                                <th width="10">
                            ...
                            <tr>
                                ...
                                <td>
                                    <select class="search" strict="true">
                                        <option value>{{ trans('global.all') }}</option>
                                        @foreach(App\SrtPeDocument::SPECIAL_COMMAND_SELECT as $key => $item)
...
@can('srt_pe_document_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.srt-pe-documents.massDestroy') }}",
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    columns: [
      ...
      { data: 'refer_documents.document_number', name: 'refer_documents.document_number', sortable: false },
