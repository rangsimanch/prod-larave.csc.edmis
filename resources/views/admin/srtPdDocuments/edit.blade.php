        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.srtPdDocument.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.srt-pd-documents.update", [$srtPdDocument->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('refer_documents') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="refer_documents_id" id="refer_documents_id" required>
                                @foreach($refer_documents as $id => $refer_documents)
                                    <option value="{{ $id }}" {{ (old('refer_documents_id') ? old('refer_documents_id') : $srtPdDocument->refer_documents->id ?? '') == $id ? 'selected' : '' }}>{{ $refer_documents }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('process_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="process_date" id="process_date" value="{{ old('process_date', $srtPdDocument->process_date) }}">
                        ...
                        <div class="form-group {{ $errors->has('special_command') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="special_command" id="special_command">
                                ...
                                @foreach(App\SrtPdDocument::SPECIAL_COMMAND_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('special_command', $srtPdDocument->special_command) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('finished_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="finished_date" id="finished_date" value="{{ old('finished_date', $srtPdDocument->finished_date) }}">
                        ...
                        <div class="form-group {{ $errors->has('operators') ? 'has-error' : '' }}">
                            <label for="operators">{{ trans('cruds.srtPdDocument.fields.operator') }}</label>
                            ...
                            <select class="form-control select2" name="operators[]" id="operators" multiple>
                                @foreach($operators as $id => $operator)
                                    <option value="{{ $id }}" {{ (in_array($id, old('operators', [])) || $srtPdDocument->operators->contains($id)) ? 'selected' : '' }}>{{ $operator }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('operators'))
                                <span class="help-block" role="alert">{{ $errors->first('operators') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.srtPdDocument.fields.operator_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('practice_notes') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="practice_notes" id="practice_notes">{{ old('practice_notes', $srtPdDocument->practice_notes) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('note') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="note" id="note" value="{{ old('note', $srtPdDocument->note) }}">
                            @if($errors->has('note'))
                        ...
                        <div class="form-group {{ $errors->has('file_upload') ? 'has-error' : '' }}">
                            <label class="required" for="file_upload">{{ trans('cruds.srtPdDocument.fields.file_upload') }}</label>
                        ...
                        <div class="form-group {{ $errors->has('save_for') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="save_for" id="save_for" required>
                                ...
                                @foreach(App\SrtPdDocument::SAVE_FOR_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('save_for', $srtPdDocument->save_for) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
