        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.letterOutgoingCec.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.letter-outgoing-cecs.update", [$letterOutgoingCec->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('letter') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="letter_id" id="letter_id">
                                @foreach($letters as $id => $letter)
                                    <option value="{{ $id }}" {{ ($letterOutgoingCec->letter ? $letterOutgoingCec->letter->id : old('letter_id')) == $id ? 'selected' : '' }}>{{ $letter }}</option>
                                @endforeach
                            </select>
