        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.dailyReport.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.daily-reports.update", [$dailyReport->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('input_date') ? 'has-error' : '' }}">
                            <label for="input_date">{{ trans('cruds.dailyReport.fields.input_date') }}</label>
                            <input class="form-control date" type="text" name="input_date" id="input_date" value="{{ old('input_date', $dailyReport->input_date) }}">
                        ...
                        <div class="form-group {{ $errors->has('acknowledge') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="acknowledge" id="acknowledge" value="1" {{ $dailyReport->acknowledge || old('acknowledge', 0) === 1 ? 'checked' : '' }}>
                                <label for="acknowledge" style="font-weight: 400">{{ trans('cruds.dailyReport.fields.acknowledge') }}</label>
                            </div>
