        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.documentTag.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.document-tags.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('tag_name') ? 'has-error' : '' }}">
                            <label for="tag_name">{{ trans('cruds.documentTag.fields.tag_name') }}</label>
                            <input class="form-control" type="text" name="tag_name" id="tag_name" value="{{ old('tag_name', '') }}">
