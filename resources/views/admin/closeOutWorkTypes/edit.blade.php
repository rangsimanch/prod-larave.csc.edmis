        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.closeOutWorkType.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.close-out-work-types.update", [$closeOutWorkType->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('work_type_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="work_type_name" id="work_type_name" value="{{ old('work_type_name', $closeOutWorkType->work_type_name) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('work_type_code') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="work_type_code" id="work_type_code" value="{{ old('work_type_code', $closeOutWorkType->work_type_code) }}">
                        ...
                        <div class="form-group {{ $errors->has('construction_contract') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="construction_contract_id" id="construction_contract_id" required>
                                @foreach($construction_contracts as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('construction_contract_id') ? old('construction_contract_id') : $closeOutWorkType->construction_contract->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
