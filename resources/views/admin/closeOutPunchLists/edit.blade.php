        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.closeOutPunchList.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.close-out-punch-lists.update", [$closeOutPunchList->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <legend><a onclick="HideSection(1)" id="element1"><i class="bi bi-eye-slash-fill"></i></a> Section 1 : Create Punchlist </legend>
                        <div id="section1">

                        <div class="form-group {{ $errors->has('subject') ? 'has-error' : '' }}">
                            <label class="required" for="subject">{{ trans('cruds.closeOutPunchList.fields.subject') }}</label>
                            <input class="form-control" type="text" name="subject" id="subject" value="{{ old('subject', $closeOutPunchList->subject) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('document_number') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="document_number" id="document_number" value="{{ old('document_number', $closeOutPunchList->document_number) }}">
                        ...
                        <div class="form-group {{ $errors->has('submit_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="submit_date" id="submit_date" value="{{ old('submit_date', $closeOutPunchList->submit_date) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('boqs') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="boqs[]" id="boqs" multiple required>
                                @foreach($boqs as $id => $boq)
                                    <option value="{{ $id }}" {{ (in_array($id, old('boqs', [])) || $closeOutPunchList->boqs->contains($id)) ? 'selected' : '' }}>{{ $boq }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('boqs'))
                        ...
                        <div class="form-group {{ $errors->has('locations') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="locations[]" id="locations" multiple>
                                @foreach($locations as $id => $location)
                                    <option value="{{ $id }}" {{ (in_array($id, old('locations', [])) || $closeOutPunchList->locations->contains($id)) ? 'selected' : '' }}>{{ $location }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('sub_location') ? 'has-error' : '' }}">
                            ...
                            <input data-role="tagsinput" class="form-control" name="sub_location" id="sub_location" value="{{ old('sub_location', $closeOutPunchList->sub_location) }}">
                        ...
                        <div class="form-group {{ $errors->has('work_types') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="work_types[]" id="work_types" multiple>
                                @foreach($work_types as $id => $work_type)
                                    <option value="{{ $id }}" {{ (in_array($id, old('work_types', [])) || $closeOutPunchList->work_types->contains($id)) ? 'selected' : '' }}>{{ $work_type }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('sub_worktype') ? 'has-error' : '' }}">
                            ...
                            <input data-role="tagsinput" class="form-control" name="sub_worktype" id="sub_worktype" value="{{ old('sub_worktype', $closeOutPunchList->sub_worktype) }}">
                        ...
                        <div class="form-group {{ $errors->has('respond_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="respond_date" id="respond_date" value="{{ old('respond_date', $closeOutPunchList->respond_date) }}">
                    ...
                    <legend><a onclick="HideSection(2)" id="element2"><i class="bi bi-eye"></i></a> Section 2 : Corective Punchlist </legend>
                        ...
                        <div class="form-group {{ $errors->has('files_report_after') ? 'has-error' : '' }}">
                            <label class="required" for="files_report_after">{{ trans('cruds.closeOutPunchList.fields.files_report_after') }}</label>
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.closeOutPunchList.fields.files_report_after_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('review_status') ? 'has-error' : '' }}">
                            <label class="required">{{ trans('cruds.closeOutPunchList.fields.review_status') }}</label>
                            <select class="form-control" name="review_status" id="review_status">
                                ...
                                @foreach(App\CloseOutPunchList::REVIEW_STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('review_status', $closeOutPunchList->review_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('review_status'))
                                <span class="help-block" role="alert">{{ $errors->first('review_status') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.closeOutPunchList.fields.review_status_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('review_date') ? 'has-error' : '' }}">
                            <label class="required" for="review_date">{{ trans('cruds.closeOutPunchList.fields.review_date') }}</label>
                            <input class="form-control date" type="text" name="review_date" id="review_date" value="{{ old('review_date', $closeOutPunchList->review_date) }}">
                            @if($errors->has('review_date'))
                                <span class="help-block" role="alert">{{ $errors->first('review_date') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.closeOutPunchList.fields.review_date_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('reviewer') ? 'has-error' : '' }}">
                            <label for="reviewer_id">{{ trans('cruds.closeOutPunchList.fields.reviewer') }}</label>
                            <select class="form-control select2" name="reviewer_id" id="reviewer_id">
                                @foreach($reviewers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('reviewer_id') ? old('reviewer_id') : $closeOutPunchList->reviewer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('reviewer'))
                                <span class="help-block" role="alert">{{ $errors->first('reviewer') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.closeOutPunchList.fields.reviewer_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('document_status') ? 'has-error' : '' }}">
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.closeOutPunchList.fields.document_status_helper') }}</span>
                        </div>
                    </div>
