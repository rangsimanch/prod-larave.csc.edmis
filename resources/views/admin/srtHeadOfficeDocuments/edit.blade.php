        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.srtHeadOfficeDocument.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.srt-head-office-documents.update", [$srtHeadOfficeDocument->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('refer_documents') ? 'has-error' : '' }}">
                            ...
                            <select readonly class="form-control select2" name="refer_documents_id" id="refer_documents_id" required>
                                @foreach($refer_documents as $id => $refer_documents)
                                    <option value="{{ $id }}" {{ (old('refer_documents_id') ? old('refer_documents_id') : $srtHeadOfficeDocument->refer_documents->id ?? '') == $id ? 'selected' : '' }}>{{ $refer_documents }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('process_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="process_date" id="process_date" value="{{ old('process_date', $srtHeadOfficeDocument->process_date) }}">
                        ...
                        <div class="form-group {{ $errors->has('finished_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="finished_date" id="finished_date" value="{{ old('finished_date', $srtHeadOfficeDocument->finished_date) }}">
                        ...
                        <div class="form-group {{ $errors->has('note') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="note" id="note" value="{{ old('note', $srtHeadOfficeDocument->note) }}">
                            @if($errors->has('note'))
                        ...
                        <div class="form-group {{ $errors->has('file_upload') ? 'has-error' : '' }}">
                            <label class="required" for="file_upload">{{ trans('cruds.srtHeadOfficeDocument.fields.file_upload') }}</label>
