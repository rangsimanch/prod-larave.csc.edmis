        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.meetingMonthly.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.meeting-monthlies.update", [$meetingMonthly->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('meeting_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="meeting_name" id="meeting_name" value="{{ old('meeting_name', $meetingMonthly->meeting_name) }}">
                        ...
                        <div class="form-group {{ $errors->has('meeting_no') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="meeting_no" id="meeting_no" value="{{ old('meeting_no', $meetingMonthly->meeting_no) }}">
                        ...
                        <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="date" id="date" value="{{ old('date', $meetingMonthly->date) }}">
                            @if($errors->has('date'))
                        ...
                        <div class="form-group {{ $errors->has('construction_contract') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="construction_contract_id" id="construction_contract_id">
                                @foreach($construction_contracts as $id => $construction_contract)
                                    <option value="{{ $id }}" {{ ($meetingMonthly->construction_contract ? $meetingMonthly->construction_contract->id : old('construction_contract_id')) == $id ? 'selected' : '' }}>{{ $construction_contract }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('note') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="note" id="note">{{ old('note', $meetingMonthly->note) }}</textarea>
                            @if($errors->has('note'))
