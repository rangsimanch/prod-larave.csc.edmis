        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.submittalsRfa.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.submittals-rfas.update", [$submittalsRfa->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('item_no') ? 'has-error' : '' }}">
                            <label for="item_no">{{ trans('cruds.submittalsRfa.fields.item_no') }}</label>
                            <input class="form-control" type="text" name="item_no" id="item_no" value="{{ old('item_no', $submittalsRfa->item_no) }}">
                            @if($errors->has(''))
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="description" id="description" value="{{ old('description', $submittalsRfa->description) }}">
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                            @endif
                        ...
                        <div class="form-group {{ $errors->has('qty_sets') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="qty_sets" id="qty_sets" value="{{ old('qty_sets', $submittalsRfa->qty_sets) }}" step="1">
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                            @endif
                        ...
                        <div class="form-group {{ $errors->has('review_status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="review_status_id" id="review_status_id">
                                @foreach($review_statuses as $id => $review_status)
                                    <option value="{{ $id }}" {{ ($submittalsRfa->review_status ? $submittalsRfa->review_status->id : old('review_status_id')) == $id ? 'selected' : '' }}>{{ $review_status }}</option>
                                @endforeach
                            </select>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                        ...
                        <div class="form-group {{ $errors->has('date_returned') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="date_returned" id="date_returned" value="{{ old('date_returned', $submittalsRfa->date_returned) }}">
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                            @endif
                        ...
                        <div class="form-group {{ $errors->has('remarks') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="remarks" id="remarks">{{ old('remarks', $submittalsRfa->remarks) }}</textarea>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                            @endif
                        ...
                        <div class="form-group {{ $errors->has('on_rfa') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="on_rfa_id" id="on_rfa_id">
                                @foreach($on_rfas as $id => $on_rfa)
                                    <option value="{{ $id }}" {{ ($submittalsRfa->on_rfa ? $submittalsRfa->on_rfa->id : old('on_rfa_id')) == $id ? 'selected' : '' }}>{{ $on_rfa }}</option>
                                @endforeach
                            </select>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
