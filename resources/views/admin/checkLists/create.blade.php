        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.checkList.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.check-lists.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('work_title') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="work_title" id="work_title" value="{{ old('work_title', '') }}">
                        ...
                        <div class="form-group {{ $errors->has('date_of_work_done') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="date_of_work_done" id="date_of_work_done" value="{{ old('date_of_work_done') }}">
                        ...
                        <div class="form-group {{ $errors->has('name_of_inspector') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="name_of_inspector_id" id="name_of_inspector_id">
                                @foreach($name_of_inspectors as $id => $name_of_inspector)
                                    <option value="{{ $id }}" {{ old('name_of_inspector_id') == $id ? 'selected' : '' }}>{{ $name_of_inspector }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('thai_or_chinese_work') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="thai_or_chinese_work" id="thai_or_chinese_work">
                                ...
                                @foreach(App\CheckList::THAI_OR_CHINESE_WORK_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('thai_or_chinese_work', 'Thai') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
