        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.closeOutLocation.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.close-out-locations.update", [$closeOutLocation->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('location_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="location_name" id="location_name" value="{{ old('location_name', $closeOutLocation->location_name) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('location_code') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="location_code" id="location_code" value="{{ old('location_code', $closeOutLocation->location_code) }}">
                        ...
                        <div class="form-group {{ $errors->has('construction_contract') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="construction_contract_id" id="construction_contract_id" required>
                                @foreach($construction_contracts as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('construction_contract_id') ? old('construction_contract_id') : $closeOutLocation->construction_contract->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
