...
class TeiInboxController extends Controller
{
    public function index(Request $request)
    {
        abort_if(Gate::denies('tei_inbox_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        if ($request->ajax()) {
            ...
            ->select(sprintf('%s.*', (new AddLetter)->table))
             ->where(function ($query) {
                $query->where('add_letters.receiver_id', 7)
                    ->orWhere(function ($subquery) {
                        $subquery->whereRaw('EXISTS (SELECT 1 FROM add_letter_team WHERE add_letters.id = add_letter_team.add_letter_id AND add_letter_team.team_id = 7)');
                    });
            });
            ...
            $table->editColumn('letter_type', function ($row) {
                // if($row->receiver->code == "TEI" && $row->mask_as_received == 0){
        ...
        session(['previous-url' => route('admin.tei-inboxes.index')]);
        return view('admin.teiInboxes.index', compact('letter_subject_types','teams', 'teams', 'teams', 'construction_contracts', 'users', 'users', 'teams'));
    }
}
