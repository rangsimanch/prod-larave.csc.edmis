...
class CcspInboxController extends Controller
{
    public function index(Request $request)
    {
        abort_if(Gate::denies('ccsp_inbox_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        if ($request->ajax()) {
            ...
            $table->editColumn('letter_type', function ($row) {
                // if($row->receiver->code == "CCSP" && $row->mask_as_received == 0){
        ...
        return view('admin.ccspInboxes.index', compact('letter_subject_types','teams', 'teams', 'teams', 'construction_contracts', 'users', 'users', 'teams'));
    }
}
