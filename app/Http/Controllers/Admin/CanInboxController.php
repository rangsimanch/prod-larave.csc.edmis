...
class CanInboxController extends Controller
{
    ...
    {
        abort_if(Gate::denies('can_inbox_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('letter_type', function ($row) {
                // if($row->receiver->code == "CAN" && $row->mask_as_received == 0){
        ...
        return view('admin.canInboxes.index', compact('letter_subject_types','teams', 'teams', 'teams', 'construction_contracts', 'users', 'users', 'teams'));
    }
}
