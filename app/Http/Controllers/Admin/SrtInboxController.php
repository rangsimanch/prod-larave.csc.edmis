...
class SrtInboxController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            ->select(sprintf('%s.*', (new AddLetter)->table))
             ->where(function ($query) {
                $query->where('add_letters.receiver_id', 1)
                    ->orWhere(function ($subquery) {
                        $subquery->whereRaw('EXISTS (SELECT 1 FROM add_letter_team WHERE add_letters.id = add_letter_team.add_letter_id AND add_letter_team.team_id = 1)');
                    });
            });
            ...
            $table->editColumn('letter_type', function ($row) {
                // if($row->receiver->code == "SRT" && $row->mask_as_received == 0){
            ...
            'sender','receiver', 'cc_to', 'construction_contract', 'letter_upload', 'mask_as_received', 'status']);
