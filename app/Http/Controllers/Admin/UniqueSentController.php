...
class UniqueSentController extends Controller
{
    public function index(Request $request)
    {
        abort_if(Gate::denies('unique_sent_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        if ($request->ajax()) {
        ...
        session(['previous-url' => route('admin.unique-sents.index')]);
        return view('admin.uniqueSents.index', compact('letter_subject_types','teams', 'teams', 'teams', 'construction_contracts', 'users', 'users', 'teams'));        
    }
}
