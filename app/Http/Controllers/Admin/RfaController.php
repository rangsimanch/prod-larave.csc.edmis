    ...
    public function createReportRFA(Rfa $rfa)
    {
       $submittalsRfa = SubmittalsRfa::all()->where('on_rfa_id' , $rfa->id);
        ...
        if($rfa->construction_contract->code == "C4-3"){
            ...
            if ($rfa->id > 135894 && $rfa->id <= 139398) {
                $issue_by = '( Gao Liang )';
    ...
    try {
        ...
        if($rfa->created_at < '2026-05-31'){
            ...
            if(strlen($title_en) > 150){
                $html .= "<div style=\"font-size: 9px; padding-right:180px; position:absolute;top:165px;left:75px; LINE-HEIGHT:16px;\">"
                . $title_en . "</div>";
            }
            else{
                $html .= "<div style=\"font-size: 12px; font-weight: bold; padding-right:180px; position:absolute;top:165px;left:75px; LINE-HEIGHT:16px;\">"
                . $title_en . "</div>";
            }

            $html .= "<div style=\"font-size: 12px; font-weight: bold; position:absolute;top:214px;left:52px;\">" . 'หัวข้อ:' . "</div>";
            if(strlen($title_th) > 200){
                $html .= "<div style=\"font-size: 9px;  padding-right:230px; position:absolute;top:214px;left:75px; LINE-HEIGHT:14px;\">"
                . $title_th . "</div>";
            }
            else{
