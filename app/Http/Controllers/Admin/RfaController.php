...
class RfaController extends Controller
{
    ...
    public function create()
    {
        ...
        $issuebies = User::where('team_id',3)->pluck('name', 'id')->prepend(trans('global.pleaseSelect'), '');
...
//            $data['document_number'] = 'HSR1/' . $const_code . $workcode  . '/' . $wbs3code . '/' . $wbs4code . '/' . $typecode . '/' . $code_date . '/' . $doc_number; 
    ...
    public function createReportRFA(Rfa $rfa)
    {
       ...
       if($for_statuses == 1){
        ...
        $mpdf->AddPage();
        $pagecount = $mpdf->SetSourceFile(public_path('pdf-asset/Circulation_General_All_Contract_Update.pdf'));
        $tplId = $mpdf->ImportPage($pagecount);
        $mpdf->UseTemplate($tplId);   
