    ...
    public function createReportRFA(Rfa $rfa)
    {
       ...
       if($for_statuses == 1){
        ...
        if(count($submittalsRfa) > 0){
            $mpdf->AddPage();
            $pagecount = $mpdf->SetSourceFile(public_path('pdf-asset/Submittals_Form_New.pdf'));
            $tplId = $mpdf->ImportPage($pagecount);
            $mpdf->UseTemplate($tplId);    
            ...
            foreach($submittalsRfa as $submittal){
                ...
                $htmlsub .= "<div style=\"font-size: 12px; padding-right:400px; position:absolute;top:". $top ."px;left:138px;LINE-HEIGHT:15px;\">". 
