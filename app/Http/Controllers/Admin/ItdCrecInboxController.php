    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            ->where(function ($query) {
                $query->where('add_letters.receiver_id', 19)
                    ->orWhere(function ($subquery) {
                        $subquery->whereRaw('EXISTS (SELECT 1 FROM add_letter_team WHERE add_letters.id = add_letter_team.add_letter_id AND add_letter_team.team_id = 19)');
                    });
            });
        ...
        session(['previous-url' => route('admin.itd-crec-inboxes.index')]);
        return view('admin.itdCrecInboxes.index', compact('letter_subject_types','teams', 'teams', 'teams', 'construction_contracts', 'users', 'users', 'teams'));
    }
}
