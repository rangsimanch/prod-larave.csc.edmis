...
class CivilInboxController extends Controller
{
    public function index(Request $request)
    {
        abort_if(Gate::denies('civil_inbox_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        if ($request->ajax()) {
            ...
            ->where(function ($query) {
                $query->where('add_letters.receiver_id', 6)
                    ->orWhere(function ($subquery) {
                        $subquery->whereRaw('EXISTS (SELECT 1 FROM add_letter_team WHERE add_letters.id = add_letter_team.add_letter_id AND add_letter_team.team_id = 6)');
                    });
            });
            ...
            $table->editColumn('letter_type', function ($row) {
                // if($row->receiver->code == "CIVIL" && $row->mask_as_received == 0){
        ...
        session(['previous-url' => route('admin.civil-inboxes.index')]);
        return view('admin.civilInboxes.index', compact('letter_subject_types','teams', 'teams', 'teams', 'construction_contracts', 'users', 'users', 'teams'));
    }
}
