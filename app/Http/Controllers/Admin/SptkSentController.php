...
class SptkSentController extends Controller
{
    public function index(Request $request)
    {
        abort_if(Gate::denies('sptk_sent_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        if ($request->ajax()) {
        ...
        session(['previous-url' => route('admin.sptk-sents.index')]);
        return view('admin.sptkSents.index', compact('letter_subject_types','teams', 'teams', 'teams', 'construction_contracts', 'users', 'users', 'teams'));        
    }
}
