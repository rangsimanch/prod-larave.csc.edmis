    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            ->where(function ($query) {
                $query->where('add_letters.receiver_id', 5)
                    ->orWhere(function ($subquery) {
                        $subquery->whereRaw('EXISTS (SELECT 1 FROM add_letter_team WHERE add_letters.id = add_letter_team.add_letter_id AND add_letter_team.team_id = 5)');
                    });
            });
        ...
        return view('admin.nwrInboxes.index', compact('letter_subject_types','teams', 'teams', 'teams', 'construction_contracts', 'users', 'users', 'teams'));
    }
}
